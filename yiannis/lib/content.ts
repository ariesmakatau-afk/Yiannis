...
export const aboutCopy = {
  ...
  body: "There has been a yiros shop on this corner for something like forty years. It wasn't always ours — it was a different shop, a different name, the same charcoal. Yianni took it on around 2002, stripped it back and rebuilt it into what you see now. Nobody around here can tell you the exact dates, and that's sort of the point: this place is old enough that it has stopped keeping count. What it has kept is the method. Lamb, chicken and pork over real charcoal, salad cut that morning, garlic sauce made the way it has always been made. That is the whole menu and the whole idea. Hindley Street has turned over more times than we can remember — we are still on the same corner, still carving off the same spit, for students at closing time, tradies at noon, and families who drive in from the suburbs because nowhere nearer tastes like this.",
};
