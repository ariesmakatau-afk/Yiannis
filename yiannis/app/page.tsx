...
export default function HomePage() {
  return (
    <>
      ...
      {/* PROMO STRIP — pulls from lib/parea.ts, same source as the /parea page */}
      {promo.active && (
        <Link
          href="/parea"
          className="group block bg-cobalt-dark text-white transition-colors hover:bg-cobalt"
        >
          <div className="container-page flex flex-wrap items-center justify-center gap-x-3 gap-y-1 py-3.5 text-center">
            <span className="text-xs font-semibold uppercase tracking-[0.18em] text-brass-light">
              {promo.eyebrow}
            </span>
            <span className="text-sm font-medium text-white/90">{promo.strip}</span>
            <span
              aria-hidden="true"
              className="text-sm text-white/50 transition-transform group-hover:translate-x-0.5"
            >
              &rarr;
            </span>
          </div>
